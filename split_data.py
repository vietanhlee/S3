...
def ensure_unique_target(dst_dir: Path, src_path: Path, rel_path: str) -> Path:
	...
	raise RuntimeError(f"Too many name collisions in {dst_dir}")
...
def materialize_split(
	...
	dst_root = output_dir / split_name
	dst_root.mkdir(parents=True, exist_ok=True)
	progress = tqdm(total=len(df_split), desc=f"Write {split_name}", unit="img")
...
def main() -> None:
	...
	materialize_split(df_train, "train", output_dir)
	materialize_split(df_val, "val", output_dir)
	materialize_split(df_test, "test", output_dir)
