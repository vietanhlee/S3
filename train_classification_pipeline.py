...
def main():
    ...
    # 1. Nạp danh sách dữ liệu
    split_path = Path(args.split_csv)
    if not split_path.exists():
        meta_path = Path(args.metadata_csv)
        if meta_path.exists():
            print(f"[*] Sử dụng file metadata thay thế: {meta_path}")
            df = pd.read_csv(meta_path)
        else:
            print(f"[!] LỖI: Không tìm thấy file {split_path} hay {meta_path}.")
            print("    Vui lòng chạy 'python generate_benchmark_assets.py' trước để tạo assets!")
            sys.exit(1)
    else:
        df = pd.read_csv(split_path)

    # Lọc bỏ nếu có Pterocarpus sp.
