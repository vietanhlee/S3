...
train_triplet.py
================
Metric Learning với Triplet Loss (Online Batch-Hard Triplet Mining).
...
MARGIN = 0.3            # Triplet loss margin (thông dụng với L2 normalized embeddings)
FREEZE_RATIO = 0.90
...
class PKSampler(Sampler):
	...
	Đảm bảo mỗi batch có đủ positive pairs/triplets cho online mining.
	"""
...
class MetricModel(nn.Module):
	...
	Output: vector 256 chiều đã chuẩn hóa L2.
	"""
	...
	def __init__(self, embedding_dim: int = 256, freeze_ratio: float = 0.90) -> None:
		...
		self.model_name = f"{MODEL_NAME}_triplet"
...
class OnlineTripletLoss(nn.Module):
	"""
	Triplet Loss với Online Batch-Hard Mining.
	Loss = mean( max(0, D(a,p) - D(a,n) + margin) )
	trong đó:
	  - D(a, p) là khoảng cách lớn nhất giữa anchor và các mẫu cùng class trong batch (Hardest Positive).
	  - D(a, n) là khoảng cách nhỏ nhất giữa anchor và các mẫu khác class trong batch (Hardest Negative).
	"""
	...
	def forward(self, embeddings: torch.Tensor, labels: torch.Tensor) -> torch.Tensor:
		...
		loss = F.relu(hardest_pos[valid] - hardest_neg[valid] + self.margin).mean()
		return loss
...
def plot_all_metrics_per_epoch(history: dict, output_dir: Path) -> None:
	...
	n_charts = 1 + 6 * 2 + len(clustering_cfgs)  # 1 loss + 6 pairs + 6 clustering
...
def main() -> None:
	...
	eda_split_class_distribution(
		...
		output_dir / "eda_split_triplet.png",
	)
	...
	# ── 9. Training loop ──
	print(f"\n{'=' * 60}")
	print(f"  Bắt đầu huấn luyện Triplet Loss — {EPOCHS} epochs")
