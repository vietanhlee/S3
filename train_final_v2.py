...
SPLIT_CONFIG = {
		"Afzelia africana": ("PP4", "test", "eff"),
		"Afzelia bella": ("PP4", "test", "eff"),
		"Afzelia pachyloba": ("PP1", "test", "eff"),
		"Afzelia quanzensis": ("PP4", "test", "eff"),
		...
		"Dalbergia tonkinensis": ("PP8", "test", "eff"),
...
class GradCAM:
	...
	def _forward_hook(self, module, inputs, output):
		...
		elif len(output.shape) == 4 and output.shape[-1] > output.shape[1]:
...
def save_gradcam_samples(
	...
	from utils import overlay_cam_on_image
...
def main() -> None:
	...
	try:
		...
		if api_key:
			...
			wandb.init(
				project="S3-Wood-Classification-v2",
				name=run_name,
				config=config_dict
			)
			use_wandb = True
